...
int CamCtrlState2D::keyPressed(Keyboard::key key) {
	switch (key) {
		case Keyboard::Key_I: {
			resetCamera();
...
int CamCtrlState2D::mouseMoved(const ScreenPos& pos) {
	...
	if (input.buttonDown(Mouse::LMB)) {
		if (input.keyDown(Keyboard::Key_Z)) {
			...
			if (cam.mode() == CAMERA_ORTHOGONAL)
				cam.zoom(-dy);
			else
				cam.moveForwards(-dy * PERSPECTIVE_SENSITIVITY);
			request_code |= GEOC_APP_REDISPLAY;
		}
	}
...
int CamCtrlState2D::mouseWheel(int delta) {
	if (cam.mode() == CAMERA_ORTHOGONAL)
		cam.zoom(delta);
	else
		cam.moveForwards(delta * PERSPECTIVE_SENSITIVITY);
	return GEOC_APP_REDISPLAY;
}

void CamCtrlState2D::resetCamera() {
...
void CamCtrlState2D::updateZoom() {
	if (!sceneMgr.empty()) {
...
void CamCtrlState2D::centerCamera() {
	if (!sceneMgr.empty()) {
		num scene_width = sceneMgr.width();
		num scene_height = sceneMgr.height();
