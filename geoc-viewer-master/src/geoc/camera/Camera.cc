...
Camera::Camera() : Spatial(Vector3(0, 0, 425), forward3),
                   ...
                   _far(DEFAULT_FAR),
                   m_zoom(1),
                   m_sensitivity(CAM_SENSITIVITY),
                   m_mode(CAMERA_ORTHOGONAL) {}
...
void Camera::applyProjectionMatrix() const {
	switch (m_mode) {
		...
		case CAMERA_ORTHOGONAL:
			...
			GEOC_DEBUG_CODE(default
			                : throw GEOC_EXCEPTION("Unknown mode");)
	}
}
...
void Camera::toggleMode() {
	if (m_mode == CAMERA_PERSPECTIVE)
		setMode(CAMERA_ORTHOGONAL);
	else
		setMode(CAMERA_PERSPECTIVE);
}
