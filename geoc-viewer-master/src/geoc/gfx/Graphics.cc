...
void Graphics::deleteStaticGeometry(unsigned int id) {
	...
	*id_arr = id;
	...
	if (displayList != impl->displayLists.end()) {
		...
		throw GEOC_EXCEPTION("The ID specified does not belong to any created static geometry");
}
...
void Graphics::flush() {
	//Draw points.
	glBegin(GL_POINTS);
	foreach (element<Point> p, impl->points)
		render_(*p.item);
	foreach (const Point& p, impl->point_pool)
		render_(p);
	glEnd();
	impl->points.clear();
	...
	//Draw segments.
	glBegin(GL_LINES);
	foreach (element<LineSegmentEnt> s, impl->segments)
		render_(*s.item);
	foreach (const LineSegmentEnt& s, impl->segment_pool)
		render_(s);
	glEnd();
	impl->segments.clear();
	...
	//Draw triangles.
	glBegin(GL_TRIANGLES);
	foreach (element<TriangleEnt> t, impl->triangles)
		render_(*t.item);
	foreach (const TriangleEnt& t, impl->triangle_pool)
		render_(t);
	glEnd();
	...
	//Draw circles.
	foreach (element<CircleEnt> c, impl->circles)
		render_(*c.item, impl->quadric);
	foreach (const CircleEnt& c, impl->circle_pool)
		render_(c, impl->quadric);
	impl->circles.clear();
	impl->circle_pool.clear();

	//Draw polygons.
	foreach (element<polygon> p, impl->polygons)
		render_(*p.item);
	foreach (const polygon& p, impl->polygon_pool)
		render_(p);
	impl->polygons.clear();
