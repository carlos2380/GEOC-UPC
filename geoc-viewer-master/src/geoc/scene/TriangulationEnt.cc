		...
		if (enable_3d) {
			...
			foreach (TriangleEnt& t, triangles_pruned) {
				...
				for (int i = 0; i < 3; ++i) {
					...
					if (alt < vege_threshold)
						colours.push_back(computeColour(sand, alt / sand_factor, 0.15));
					else if (alt < mntn_threshold)
						colours.push_back(computeColour(vege, (alt - vege_threshold) / vege_factor, 0.15));
					else
						colours.push_back(computeColour(mntn, (alt - mntn_threshold) - mntn_factor, 0.30));
...
void TriangulationEnt::drawLabel(const Font& f) const {
	if (geometry[drawMode] != -1) {
		if (drawMode == TRIANGULATION_2D_RAW) {
			foreach (const LineSegmentEnt& s, segments)
				s.drawLabel(f);
			foreach (const TriangleEnt& t, triangles)
				t.drawLabel(f);
		} else {
			foreach (const TriangleEnt& t, triangles_pruned)
				t.drawLabel(f);
		}
	}
}
